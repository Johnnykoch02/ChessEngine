...
class MoveGenerator:
    ...
    def GenerateLegalMoves(piece, board):
        ...
        if not MoveGenerator.is_king_in_check(piece.color, v_board):
            MoveGenerator.GetPseudoLegalMoves(
                PseudoLegalMoves, MoveTypes, piece, board, False
            )
        else:
            king_in_check = True
    ...
    def any_move_removes_check(board, piece):
        ...
        for piece in pieces:
            ...
            PseudoLegalMoves = set()
            MoveGenerator.GetPseudoLegalMoves(
                PseudoLegalMoves, Move.GetMoveSet(vPiece), vPiece, virtual_board
            )
            for move in PseudoLegalMoves:
    ...
    def is_king_in_check(current_color, board):
        if current_color == Piece.Color.WHITE:
            ...
            for piece in black_pieces:
                ...
                for move in PseudoLegalMoves:
                    if (
                        virtual_board.white_king
                        and move == virtual_board.white_king.square
                    ):
                        # print('King in Check')
                        return True
            ...
            for piece in white_pieces:
                ...
                for move in PseudoLegalMoves:
                    if (
                        virtual_board.black_king
                        and move == virtual_board.black_king.square
                    ):
                        # print('King in Check')
                        return True
    ...
    def GetPseudoLegalMoves(PseudoLegalMoves, MoveTypes, piece, board, debug=False):
        ...
        for move_type in MoveTypes:
            if move_type == NSEW:
                ...
                e = (0, 1)
                w = (0, -1)
                MoveGenerator.MoveInDirectionNorm(
                    PseudoLegalMoves,
                    piece,
                    add_two_pos(initial_pos, n),
                    board,
                    n,
                    debug,
                )  # North
            ...
            if move_type == PAWN_BLACK:
                MoveGenerator.pawn_moves(
                    PseudoLegalMoves, piece, initial_pos, board, -1
                )
            if move_type == KING:
    ...
    @staticmethod
    def MoveInDirectionNorm(
        PsuedoLegalMoves, piece, pos, board, direction, debug=False
    ):
...
class Move:
    ...
    def GetMoveSet(piece: "Piece"):
        ...
        if piece.type == Piece.Type.QUEEN or piece.type == Piece.Type.ROOK:
            ...
            (
                MoveTypes.append(PAWN_WHITE)
                if piece.color == Piece.Color.WHITE
                else MoveTypes.append(PAWN_BLACK)
            )
