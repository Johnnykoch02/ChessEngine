...
class AppManager:
    ...
    def mouse_events(self, mouse_pos, presses):
        square_on = (
            int(mouse_pos[1] // SQUARE_DIMENSIONS[0]),
            int(mouse_pos[0] // SQUARE_DIMENSIONS[0]),
        )  # Mouse to Square
        print(square_on)
        if presses[0]:
            if self.piece_selected:
                ...
                if (
                    temp_piece is not None
                    and temp_piece.color == self.current_player.type
                ):
                    self.piece_selected = temp_piece
                if self.piece_selected:
                    self.board.remove_piece(self.piece_selected)
                    self.piece_selected.selected = True
                    pseudolegal, _, _ = self.MoveGenerator.GenerateLegalMoves(
                        self.piece_selected, self.board
                    )
                    self.legal_moves = tuple(
                        tuple([int(x[0]), int(x[1])]) for x in pseudolegal
                    )
    ...
    def MCSimulate(self, n_games=1000):
        mc_sim = MCTS()
        board = self.board
        envP1 = GrandMasterEnv(
            board, self.black_player.type
        )  # TODO: Change GMEnv to take in agents as input.
        ...
        while p_games < n_games:
            ...
            _header = f"{'-'*5}\n"
            print(
                f"{_header*3}Player {c_env._team_color}: Played {piece.get_type_str()} to {piece.square}"
            )
            envs.append(c_env)
            c_env = envs.popleft()
    ...
    def DataCollection(self):
        if any(self.board.get_state(self.current_player.type)["check"][0]):
            ...
            print("Resetting...")
        _, wk_check, wk_ckm = self.MoveGenerator.GenerateLegalMoves(
            self.board.white_king, self.board
        )
        _, bk_check, bk_ckm = self.MoveGenerator.GenerateLegalMoves(
            self.board.black_king, self.board
        )
...
class DataCollector:
    ...
    def __init__(
        ...
        player,
        board,
        save_dir=os.path.join(os.getcwd(), "src", "Managers", "RL", "PretrainData"),
    ):
    ...
    def save(self):
        ...
        for i in range(self.num_records):
            ...
            np.savez(
                os.path.join(data_path, "observation.npz"),
                board_state=obs["board_state"],
                team_color=obs["team_color"],
                score=obs["score"],
                check=obs["check"],
                random_state=obs["random_state"],
            )
    ...
    def pretrain(self, batch_size=2048):
        ...
        model.save(
            os.path.join(
                os.getcwd(), "src", "Managers", "PretrainedModel", "PretrainedModel.pkl"
            )
        )
