...
class GrandMasterNetwork(nn.Module):
    def __init__(
        self,
        lr,
        checkpoint_dir="Grand_Master_v_0_0_0",
        version="0.0.0",
        name="GrandMasterNetwork",
    ):
        ...
        self.features_extractor = GrandMasterFeaturesExtractor(
        ...
        self.encoding_layers = nn.Sequential(
            nn.Linear(self.features_extractor.get_feature_dim(), 2056),
            nn.LeakyReLU(),
            nn.Linear(2056, 512),
            nn.LeakyReLU(),
        )
    ...
    def __init__(self, observation_space, device="cuda"):
        ...
        self.residuals = [
            nn.Sequential(
                ...
                nn.Tanh(),
                nn.Conv2d(
                    in_channels=32, out_channels=32, kernel_size=(4, 4), padding="same"
                ),
                nn.Tanh(),
                nn.Conv2d(
                    in_channels=32, out_channels=32, kernel_size=(5, 5), padding="same"
                ),
                nn.Tanh(),
                nn.Conv2d(
                    in_channels=32, out_channels=3, kernel_size=(3, 3), padding="same"
                ),
                nn.Tanh(),
                nn.BatchNorm2d(3),
            )
