...
commands = {
    "exit": {"fnc": sys.exit, "args": ["load_dir", "num_steps"]},
    ...
    if cmd in commands:
        if cmd == "train":
            ...
            if "num_steps" in args and "load_dir" in args:
                ...
                (
                    iAppManager.Train(num_steps=int(args["num_steps"]))
                    if "num_steps" in args
                    else iAppManager.Train(load_dir=args["load_dir"])
                )
        elif cmd == "demo":
            current_command = commands[cmd]
